...
- `landing` și `login/register` au fost verificate live ca reachability și copy, dar nu sunt încă confirmate click-by-click ca funnel public complet `register-first`
- onboarding-ul din această rundă a fost confirmat pe backend live (`user nou + user mode + profile cu CUI și website`), nu click-by-click în UI pe toate sub-pasurile wizardului
...
- `specialist_handoff` = `PARȚIAL CONFIRMAT LIVE`
  - confirmate live:
    - `GDPR-013`
    - `GDPR-014`
    - `NIS2-001`
    - `NIS2-005`
    - `NIS2-015`
  - formula confirmată live pentru aceste cazuri este:
...
### 14.6 Încă neconfirmat live

Nu sunt încă închise live cap-coadă:
- `landing -> register UI -> onboarding UI` cap-coadă fără seed API
- `revalidation`
- `reopen` după mai multe tipuri de închidere reușită reală
- `alerts / drift UI` cu seed dedicat
- `client import + client context` în partner mode pe cont fresh fără plan Partner
- `GDPR-019` specialist round-trip până înapoi în cockpit
- `NIS2-GENERIC` pe toate ramurile lui (`governance`, `vendor`, `maturity`) cu revenire automată completă
- restul familiilor `specialist_handoff` care nu sunt încă în lista confirmată mai sus
...
- primele `specialist_handoff` critice sunt confirmate live cu revenire automată în cockpit
- însă funnel-ul public complet și restul ramurilor `specialist_handoff` nu sunt încă confirmate exhaustiv
...
- onboarding-ul UI este confirmat cap-coadă live
- toate ramurile `specialist_handoff` sunt confirmate cap-coadă
