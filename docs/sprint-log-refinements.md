...
| RR-2 | Wave 3 — HR registry reconciliation v1 | 🟢 Închis | 2026-03-30 | 2026-03-30 |
| RR-3 | COOKIE-BANNER: Cookie Banner Generator — API + generator + UI card | 🟢 Închis | 2026-03-30 | 2026-03-30 |
| RR-4 | Sprint 2.3 + Sprint 3: AI Approval/Attestation, DPA-Vendor Linking, Privacy Policy GDPR Completeness, Retention Wizard, REGES Import/Export | 🟢 Închis | 2026-03-31 | 2026-03-31 |
| RR-5 | Fix import pipeline — website column + conservative findings | 🟢 Închis | 2026-04-01 | 2026-04-01 |
| RR-6 | P2 — Client context + White-label basics | 🟢 Închis | 2026-04-01 | 2026-04-01 |
| P3-B | Vendor Trust Pack — export bundled pentru procurement / due diligence | 🟢 Închis | 2026-04-01 | 2026-04-01 |
| RR-7 | P4 — Agent Control Center + Self-Learning + Predictive Risk + Regulatory Radar Phase 2 | 🟢 Închis | 2026-04-01 | 2026-04-01 |
...
> **RR-3 (2026-03-30):** Cookie Banner Generator — `lib/server/cookie-banner-generator.ts` (generator HTML/CSS/JS self-contained, categorii trackers, `window.compliCC` API, localStorage consent), `app/api/cookie-banner/generate` (POST, citește `state.siteScanJobs` pentru ultimul scan cu trackere), `components/compliscan/cookie-banner-card.tsx` (UI card interactiv cu generate/copy/reset, preview snippet, category pills), integrat în tab "Generează" din `documents-page.tsx`.
> **RR-6 (2026-04-01):** P2 — Client context + White-label basics — PR local. Fișiere: `lib/server/white-label.ts` (WhiteLabelConfig store cu Map fallback + Supabase), `app/api/partner/white-label/route.ts` (GET/PATCH partner-only), `components/compliscan/client-context-panel.tsx` (drill-down UI: score ring, findings, NIS2, vendor reviews, quick actions fără workspace switch), `app/portfolio/client/[orgId]/page.tsx` (pagină context client în portfolio mode, moștenește PortfolioLayout), `portfolio-overview-client.tsx` — drilldown actualizat: click rând → `/portfolio/client/${orgId}` (nu mai face workspace switch direct), `portfolio-reports-page.tsx` — secțiune White-label: logo URL, brand color (color picker + hex input), partner name, tagline + preview header raport în timp real. 0 erori TS non-test.
> **MF-P3-B (2026-04-01):** Vendor Readiness Pack — export nou `/api/exports/vendor-trust-pack`, builder `lib/server/vendor-trust-pack.ts`, chestionar procurement și card dedicat în `reports-page.tsx`. Pack-ul agregă GDPR + NIS2 + AI + vendor reviews într-un livrabil comercial gata de share/export.
> **MF-P3-C (2026-04-01):** AI Act Evidence Pack — `buildAIActObligationFindings()` + sync pe AI approvals, export nou `/api/exports/ai-act-evidence-pack`, card dedicat în `/dashboard/sisteme`, document `annex-iv` salvat în Dosar și obligation findings generate per sistem AI.
> **MF-P3-D (2026-04-01):** Pay Transparency Full — store dedicat pentru salarii și rapoarte, upload CSV, calculator gap salarial, pagină nouă `/dashboard/pay-transparency`, finding canonic `PAY-001`, document `pay-gap-report` și trecere în `under_monitoring` cu review cycle programat.
> **MF-P3-E (2026-04-01):** Drift Maturation — store `review-cycle-store`, engine `drift-trigger-engine`, trigger-e legate în `org/profile`, `ai-systems`, `nis2/vendors`, `nis2/incidents`, `fiscal/protocol`, widget `Drift activ` în dashboard și cron nou `/api/cron/drift-sweep` cu auto-reopen după grace period pentru finding-urile cu policy strict.
> **RR-7 (2026-04-01):** P4 — Agent Control Center + Self-Learning + Predictive Risk + Regulatory Radar Phase 2. Specs 12.13–12.16 scrise în `implementation-specs.md`. Implementat: `lib/server/agent-feedback-store.ts` (feedback loop: approved/rejected/ignored, approvalRate, adjustedConfidence), `lib/server/eurlex-client.ts` (EUR-Lex SPARQL API, fetch acte recente NIS2/AI Act/GDPR/e-Factura), `lib/server/dnsc-monitor.ts` (DNSC.ro HTML scraping, impact detection), `lib/compliance/risk-trajectory.ts` (predicție deterministă scor 7/30/90 zile, iminentRisks, trend), `components/compliscan/risk-trajectory-widget.tsx` (widget dashboard: scor acum→30 zile, sparkline, top riscuri, CTA), `app/api/risk/trajectory/route.ts` (GET endpoint trajectory), `app/api/agents/route.ts` (PATCH feedback endpoint), `lib/server/agent-orchestrator.ts` (AgentExecutionOptions cu externalSources), `lib/compliance/agent-regulatory-radar.ts` (Phase 2: bloc surse externe EUR-Lex + DNSC), `app/api/cron/agent-regulatory-radar/route.ts` (pre-fetch EUR-Lex + DNSC înainte de run), `components/compliscan/navigation.ts` (agenti nav item în secțiunea Automatizare), `app/dashboard/page.tsx` (RiskTrajectoryWidget adăugat). 0 erori TS în fișierele noi/modificate.
> **MF-P0/P1-HARDEN (2026-04-01):** Suprafețe operaționale adăugate peste foundation-ul deja existent — `review_cycles` are acum API dedicat (`/api/review-cycles`, `/api/review-cycles/[id]`) și pagină nouă `/dashboard/review` pentru reverificări, completare și amânare; `scheduled_reports` primește rută clară în dashboard (`/dashboard/settings/scheduled-reports`), selecție reală de firme din portofoliu, fallback corect pe `session.orgId` în API și istoric minim al rulărilor prin event log-ul organizației.
> **RR-5 (2026-04-01):** Fix import pipeline — website column + conservative findings. Cauza radăcină: firmele importate arătau 0 riscuri (parser nu recunoștea `website_url`, baseline-scan genera minimal findings din ANAF prefill). Fix: (1) `import-parser.ts` — `website` adăugat ca al 6-lea `ImportColumnId` cu aliases `website_url`, `site`, `url`, etc.; (2) `execute/route.ts` — `website: string | null` adăugat la `ConfirmedRow`, salvat în `OrgProfile`; (3) `import-wizard.tsx` — `website` adăugat în `COLUMN_LABELS`, state init, `confirmedRows` mapping și payload baseline-scan; (4) `baseline-scan/route.ts` — Phase B rescris complet cu `conservativeAnswers` (worst-case: `processesPersonalData="probably"`, `hasPrivacyPolicy="no"`, `hasRopaRegistry="no"`, etc.) → 15-20+ findings per firmă importată. Merged main + deploy Vercel prod. 0 erori TS în fișierele modificate.
> **RR-4 (2026-03-31):** Sprint 2.3 + Sprint 3 batch — 5 features: (1) AI System Approval + Attestation — `AISystemApprovalStatus`/`AISystemAttestationStatus` types, PATCH `/api/ai-systems` endpoint, approve/reject + attest buttons per system card; (2) DPA-Vendor Linking — `fingerprintMatch()` din vendor-library surfaced pe AI system cards, badge "DPA public" cu link sau "DPA nedisponibil" warning; (3) Privacy Policy GDPR Art. 13/14 Completeness — 10 cerințe semantice (identitate operator, DPO, scopuri, temei juridic, destinatari, retenție, drepturi, retragere, plângere ANSPDCP, transferuri internaționale) integrate în `validateGeneratedDocumentEvidence()`; (4) Retention Wizard — matrice structurată (categorie/perioadă/trigger/bază legală) în generator-drawer, auto-formatare în context field; (5) REGES CSV Import/Export — import CSV cu detecție fuzzy coloane RO (Nume/Prenume/Funcție/Contract/Data angajare), export snapshot ca CSV cu BOM UTF-8. Fișiere: `lib/compliance/types.ts`, `app/api/ai-systems/route.ts`, `components/compliscan/ai-inventory-panel.tsx`, `components/compliscan/use-cockpit.tsx`, `lib/compliscan/generated-document-validation.ts`, `lib/compliscan/generated-document-validation.test.ts`, `components/compliscan/generator-drawer.tsx`, `components/compliscan/hr-reges-reconciliation-card.tsx`, `app/dashboard/sisteme/page.tsx`. 0 erori TS non-test. 4/4 teste passed.
> **RR-2 (2026-03-30):** Wave 3 — HR registry reconciliation v1. `GDPR-023` nu se mai bazează doar pe brief-ul REGES: suprafața `Documente` are acum card de reconciliere `snapshot intern vs REGES`, API dedicat (`/api/hr/reconciliation`), persistare în state și notă de handoff dinamică întoarsă în cockpit când snapshotul și checklistul sunt salvate. Nu pretindem încă integrare externă HRIS / REGES; acesta este pasul de comparație și urmă internă.
...
| 2026-03-29 | Codex | Pivot fiscal deschis după Wave 1 docs: preluăm din groundwork-ul Gemini doar `efactura-xml-repair.ts` și tipurile aferente, fără să amestecăm `deletion-attestation`. Ținta este `Fiscal Pre-Validator v1`: `EF-003/EF-005 -> cockpit -> /dashboard/fiscal?tab=validator -> validate -> repair safe -> dovadă/export`. |
| 2026-03-29 | Codex | `Fiscal Pre-Validator v1` implementat: API nou `POST /api/efactura/repair`, validatorul XML montat în `/dashboard/fiscal`, handoff canonic din cockpit pentru `EF-003` și `EF-005`, plus helper de reparare sigură pentru codurile `T003`, `V002`, `V003`, `V006`. |
| 2026-03-29 | Codex | Bug live prins și corectat în validator: UI filtra greșit codurile și trimitea spre repair doar subsetul din `latestValidation.errors`, ratând fixurile sigure. Corecția canonică a fost să apelăm repair pe XML-ul curent și să lăsăm helperul să aplice fallback-ul controlat pe toate fixurile sigure disponibile. |
| 2026-03-29 | Codex | Smoke live final reușit pe aliasul public `https://compliscanag.vercel.app`: `returnedToValidator=true`, `validationStatusVisible=true`, `repairPanelVisible=true`, `repairedXmlHasHeader=true`, `repairedXmlHasCius=true`, `repairedXmlHasInvoiceTypeCode380=true`. Artefactele temporare au rămas doar în `/tmp` pentru audit și se curăță după închiderea feature-ului. |
| 2026-03-29 | Codex | `Fiscal Pre-Validator v2` deschis ca polish de follow-up, fără extindere de backend: păstrăm aceleași coduri sigure, dar facem validatorul util după repair. Scope: `download repaired XML`, `copy note for cockpit`, `protocol după repair` și copy clar în bannerul de handoff fiscal. |
| 2026-03-29 | Codex | `Fiscal Pre-Validator v2` livrat în UI: validatorul afișează acum buton de descărcare pentru XML-ul reparat, buton de copiere a notei pentru cockpit și protocol explicit de retransmitere / revenire în finding. Commit `d8fe380` pe `main`. |
| 2026-03-29 | Codex | Verificare live reușită după deploy curat `dpl_8j8Ych7X2CJpKTenXYN2ZxkyHw2H` și aliasare pe `https://compliscanag.vercel.app`: `downloadButtonVisible=true`, `repairSummaryButtonVisible=true`, `protocolVisible=true`, iar XML-ul reparat păstrează headerul, `CIUS-RO:1.0.1` și `InvoiceTypeCode 380`. |
| 2026-03-30 | Codex | `Fiscal Status Interpreter` deschis pentru `EF-004` și `EF-005`: noul tab `Protocol fiscal` din `/dashboard/fiscal`, workflow link canonic din cockpit, notă pregătită pentru revenire și handoff secundar spre `SPV Check` / `Validator XML`. |
| 2026-03-30 | Codex | Validarea de deploy pentru `Fiscal Status Interpreter` este încă în curs. Am prins și documentat două build gates nelegate de sprint: dependență lipsă în exportul temporar `audit-pack-pdf.ts` și erori ESLint `no-explicit-any` în `app/api/org/profile/route.test.ts`. Unblockerul minim pentru test a fost aplicat doar ca să putem continua validarea. |
| 2026-03-30 | Codex | Wave 3 deschis cu `Circuit de semnare v1`, fără integrare externă și fără să schimbăm gating-ul finding-ului: întâi reparăm fundația (`normalizeGeneratedDocuments` acceptă toate tipurile noi și `contract-template` intră și în semnalele de prefill), apoi adăugăm urmă bilaterală doar pentru `dpa` și `contract-template`. |
| 2026-03-30 | Codex | `Circuit de semnare v1` implementat în produs: documentele `dpa` și `contract-template` capătă stări persistente `revizuit intern -> trimis la semnare -> semnat -> pus în uz`, salvabile prin API dedicat și vizibile în cockpit, Dosar și Vault. La intrarea documentului în Dosar, starea se inițializează onest cu `revizuit intern`; semnarea externă rămâne explicit în afara produsului și nu este pretinsă automat. |
| 2026-03-30 | Codex | `Wave 3 — HR registry reconciliation v1` implementat: pentru `GDPR-023`, brief-ul REGES este completat acum de o suprafață de reconciliere `snapshot intern vs REGES`, salvată în state și întoarsă în cockpit cu o notă dinamică mai clară. V1 rămâne onest: nu pretinde import automat din HRIS sau operare directă în registru. |
| 2026-03-30 | Codex | `Wave 3 — Integrări fiscale mai adânci v1` implementat: în tabul `Protocol fiscal` pentru `EF-004` și `EF-005` există acum `Jurnal de execuție fiscală`, cu persistență pentru referința facturii, statusul operațional, referința SPV, locația dovezii și nota operatorului. Întoarcerea în cockpit folosește acum o notă dinamică bazată pe execuția reală, nu doar șablonul static al protocolului. |
| 2026-03-30 | Codex | `Wave 3 — Integrări fiscale mai adânci v2` implementat: jurnalul fiscal pentru `EF-004` și `EF-005` urmărește acum recipisa SPV în mod structurat (`missing / received / accepted / rejected`) plus momentul primirii. Nota întoarsă în cockpit include verdictul recipisei, iar readiness-ul protocolului nu mai tratează lipsa recipisei ca detaliu opțional după transmitere / retransmitere. |
| 2026-03-30 | Codex | `Wave 3 — Vendor lifecycle v1` deschis pe suprafața existentă `Vendor Review`, fără motor nou paralel: păstrăm cron-ul, starea `overdue-review` și revalidarea deja existente, iar lipsa reală pe UX este un radar de follow-up pentru review-urile expirate, cele care expiră curând și cele rămase în execuție. |
| 2026-03-30 | Codex | `Wave 3 — Vendor lifecycle v1` implementat: `Vendor Review` are acum `Radar lifecycle vendor`, construit peste lifecycle-ul existent (`nextReviewDueISO`, `overdue-review`, follow-up activ), plus o notă de reminder gata de copiat pentru execuția de follow-up. V1 nu promite remindere externe automate; aduce vizibilitate și prioritate operațională pe review-urile deja existente. |
| 2026-03-30 | Codex | `Wave 3 — Vendor lifecycle v2` implementat: review-urile vendor active pot avea acum `follow-up` programat explicit (termen + notă), salvat prin `PATCH /api/vendor-review/:id` și vizibil atât în `ReviewPanel`, cât și în `Radar lifecycle vendor`. V2 nu trimite remindere externe, dar închide o gaură reală: review-urile deschise nu mai rămân fără următorul touch operațional. |
| 2026-04-01 | Codex | `P3-B — Vendor Trust Pack` implementat pe branchul `codex/p0-p3-freeze`: builder server-side nou pentru readiness comercial bundled, questionnaire auto-completat pentru due diligence, endpoint de export `GET /api/exports/vendor-trust-pack` cu JSON și PDF simplu branded, plus card nou vizibil în `Rapoarte`. Sprintul reutilizează adevărul existent din state, NIS2, vendor reviews, AI inventory și share token, fără model paralel nou. |
