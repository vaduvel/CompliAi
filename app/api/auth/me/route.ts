...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Pull icpSegment din white-label config pentru sidebar HR-aware
    let icpSegment: IcpSegment | null = null
    try {
      const wl = await getWhiteLabelConfig(session.orgId)
      icpSegment = wl.icpSegment
    } catch {
      // White-label fetch failure non-blocking; sidebar fallback la default
    }
