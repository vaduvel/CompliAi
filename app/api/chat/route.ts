...
function buildAssistantAnswer(
  ...
  if (lower.includes("ce fac") || lower.includes("priorit")) {
    return [
      `Ce am detectat`,
      `${leadFinding?.title ?? "Există alerte active care cer prioritate operațională."}`,
      ``,
      `De ce contează`,
      `${leadFinding?.excerpt ?? `Ai ${context.redAlerts} alerte roșii deschise, iar acestea trag în jos scorul de risc (${context.score}% - ${context.riskLabel}).`}`,
      ``,
      `Ce verifici acum`,
      `1) Închide mai întâi alertele roșii (${context.redAlerts}).`,
  ...
  return [
    ...
    ``,
    `De ce contează`,
    `Scorul de risc este ${context.score}% (${context.riskLabel}) și ai ${context.alerts} alerte active.`,
    ``,
    `Ce verifici acum`,
