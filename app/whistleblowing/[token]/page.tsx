...
export default function WhistleblowingSubmitPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  if (submitted) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-eos-bg p-4">
        <div className="w-full max-w-md rounded-eos-lg border border-eos-border bg-eos-surface p-8 text-center shadow-sm">
          <CheckCircle2 className="mx-auto size-12 text-eos-success" strokeWidth={1.5} />
          <h1 className="mt-4 text-xl font-semibold text-eos-text">Sesizare înregistrată</h1>
          ...
          <p className="mt-4 text-xs text-eos-text-tertiary">Poți închide această pagină.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-eos-bg p-4">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-3 text-xl font-semibold text-eos-text">Canal de sesizare</h1>
          <p className="mt-1 text-sm text-eos-text-muted">
            Sesizările sunt tratate confidențial. Poți rămâne anonim.
          </p>
        </div>

        <div className="rounded-eos-lg border border-eos-border bg-eos-surface p-6 shadow-sm space-y-5">
          <div>
            <label className="block text-xs font-medium text-eos-text-muted mb-1">Categorie</label>
            <select
              className="w-full rounded-eos-md border border-eos-border-strong bg-eos-surface px-3 py-2 text-sm text-eos-text outline-none focus:border-eos-primary focus:ring-1 focus:ring-blue-500"
              value={category}
          ...
          <div>
            <label className="block text-xs font-medium text-eos-text-muted mb-1">Descriere (minim 20 caractere)</label>
            <textarea
              className="w-full rounded-eos-md border border-eos-border-strong px-3 py-2 text-sm text-eos-text outline-none focus:border-eos-primary focus:ring-1 focus:ring-blue-500"
              rows={5}
          ...
          <div className="rounded-eos-md border border-eos-border bg-eos-bg-inset p-4 space-y-3">
            <label className="flex items-start gap-3 cursor-pointer">
              <input
                type="checkbox"
                className="mt-0.5 size-4 rounded border-eos-border-strong accent-eos-primary"
                checked={anonymous}
                onChange={(e) => setAnonymous(e.target.checked)}
              />
              <div>
                <p className="text-sm font-medium text-eos-text flex items-center gap-1.5">
                  <Shield className="size-3.5 text-eos-primary" />
                  Sesizare anonimă
                </p>
                <p className="text-xs text-eos-text-muted">Datele tale nu vor fi salvate și nu vei putea fi identificat.</p>
              </div>
            </label>
            {!anonymous && (
              <div>
                <label className="block text-xs font-medium text-eos-text-muted mb-1">Date contact (opțional)</label>
                <input
                  className="w-full rounded-eos-md border border-eos-border-strong px-3 py-2 text-sm text-eos-text outline-none focus:border-eos-primary"
