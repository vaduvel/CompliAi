  ...
  async function handleAccept(event: React.FormEvent) {
    ...
    if (!currentUserEmail) {
      if (!password || password.length < 8) {
        setError("Alege o parola de cel putin 8 caractere pentru a revendica organizatia.")
        return
      }
        ...
        <Card className="border-eos-border bg-eos-surface">
          <CardHeader className="border-b border-eos-border pb-5">
            <CardTitle className="text-xl">
              {done ? "Ownership revendicat" : "Revendica organizatia"}
            </CardTitle>
            ...
            {done ? (
              <div className="space-y-4 text-center">
                ...
                <p className="text-sm text-eos-text">
                  Ownership-ul a fost revendicat cu succes. Vei fi dus direct in dashboard.
                </p>
              </div>
            ) : (
              <form onSubmit={(event) => void handleAccept(event)} className="space-y-4">
                {!token && (
                  <div className="rounded-eos-md border border-eos-error-border bg-eos-error-soft px-4 py-3 text-sm text-eos-error">
                    Link-ul de claim este invalid sau lipseste tokenul.
                  </div>
                )}

                {loadingCurrentUser ? (
                  <div className="rounded-eos-md border border-eos-border bg-eos-surface-variant px-4 py-3 text-sm text-eos-text-muted">
                    Verificam daca ai deja o sesiune activa...
                  </div>
                ) : currentUserEmail ? (
                  <div className="rounded-eos-md border border-eos-border bg-eos-surface-variant px-4 py-3 text-sm text-eos-text-muted">
                    Vei revendica organizatia cu sesiunea activa: <strong>{currentUserEmail}</strong>
                  </div>
                ) : (
                  <>
                    <div className="rounded-eos-md border border-eos-border bg-eos-surface-variant px-4 py-3 text-sm text-eos-text-muted">
                      Daca nu ai inca acces, alege acum parola pentru contul care va deveni owner.
                    </div>
