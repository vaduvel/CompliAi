-- ============================================================
-- CompliAI (CompliScan) — Schema complet Supabase
-- Rulează o singură dată în SQL Editor → New Query
-- Toate comenzile sunt idempotente (IF NOT EXISTS / OR REPLACE)
-- ============================================================


-- ────────────────────────────────────────────────────────────
-- BLOC 1: Schema legacy + tabel principal org state
-- ────────────────────────────────────────────────────────────
...
for each row execute function compliscan.set_updated_at();
...
on compliscan.app_state as permissive for all to service_role
using (true) with check (true);


-- ────────────────────────────────────────────────────────────
-- BLOC 2: Tabele principale (organizations, profiles, memberships,
--         org_state, evidence_objects) + RLS + Storage bucket
-- ────────────────────────────────────────────────────────────
...
returns text language sql immutable set search_path = public, storage as $$
...
using (id in (select org_id from public.current_user_org_ids()));
...
create policy org_state_member_select on public.org_state for select to authenticated
using (org_id in (select org_id from public.current_user_org_ids()));
...
create policy service_role_full_access_organizations on public.organizations as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_profiles on public.profiles;
create policy service_role_full_access_profiles on public.profiles as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_memberships on public.memberships;
create policy service_role_full_access_memberships on public.memberships as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_org_state on public.org_state;
create policy service_role_full_access_org_state on public.org_state as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_evidence_objects on public.evidence_objects;
create policy service_role_full_access_evidence_objects on public.evidence_objects as permissive for all to service_role using (true) with check (true);

-- Storage bucket pentru evidence
...
create policy evidence_bucket_authenticated_select on storage.objects for select to authenticated
using (
...
create policy evidence_bucket_service_role_all on storage.objects as permissive for all to service_role
using (bucket_id = 'compliscan-evidence-private')
with check (bucket_id = 'compliscan-evidence-private');


-- ────────────────────────────────────────────────────────────
-- BLOC 3: Tabele runtime (plans, nis2_state, notifications,
--         agent_runs, vendor_reviews)
-- ────────────────────────────────────────────────────────────
...
create trigger notifications_state_set_updated_at before update on public.notifications_state for each row execute function public.set_updated_at();

drop trigger if exists nis2_state_set_updated_at on public.nis2_state;
create trigger nis2_state_set_updated_at before update on public.nis2_state for each row execute function public.set_updated_at();
...
create trigger vendor_reviews_set_updated_at before update on public.vendor_reviews for each row execute function public.set_updated_at();
...
create policy nis2_state_member_select on public.nis2_state for select to authenticated
using (org_id in (select org_id from public.current_user_org_ids()));
...
create policy service_role_full_access_notifications_state on public.notifications_state as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_nis2_state on public.nis2_state;
create policy service_role_full_access_nis2_state on public.nis2_state as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_agent_runs on public.agent_runs;
create policy service_role_full_access_agent_runs on public.agent_runs as permissive for all to service_role using (true) with check (true);

drop policy if exists service_role_full_access_vendor_reviews on public.vendor_reviews;
create policy service_role_full_access_vendor_reviews on public.vendor_reviews as permissive for all to service_role using (true) with check (true);


-- ────────────────────────────────────────────────────────────
-- BLOC 4: DSAR state + vendor_review_state
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- BLOC 5: Score snapshots (trend zilnic)
-- ────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────
-- BLOC 6: Legislation hashes (radar legislativ)
-- ────────────────────────────────────────────────────────────

create table if not exists public.legislation_hashes (
...
drop policy if exists anaf_tokens_org_isolation on public.anaf_tokens;
create policy anaf_tokens_org_isolation on public.anaf_tokens
  as permissive for all to service_role
  using (true) with check (true);


-- ────────────────────────────────────────────────────────────
-- GATA — toate tabelele au fost create/actualizate.
-- ────────────────────────────────────────────────────────────
